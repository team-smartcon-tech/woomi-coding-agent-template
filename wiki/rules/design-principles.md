...
- **본문 폭을 제한해 중앙 정렬하지 않는다.** 표·대시보드는 폭이 곧 정보량이다. 폼처럼 넓으면 읽기 나쁜 화면만 그 화면에서 감싼다 — 전역 규칙 하나로 두 요구를 다 만족시킬 수 없다.
- **로고 이미지 위에 색을 덮지 않는다.** 배지 배경·`filter`·`color` 어느 쪽도 안 된다. 로고 색과 UI 토큰은 다른 값이다. 좁은 폭에서는 가로형 로고를 줄이지 말고 정사각 심볼로 **교체**한다.
