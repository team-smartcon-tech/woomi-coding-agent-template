...
- 문구·스타일·명확한 단일 파일 수정은 대상 코드 확인 후 구현·검증한다.
- 여러 모듈, 새 기능, API 소비자 변경, 원인이 불명확한 실패는 영향·계약·검증을 짧게 계획한다.
- 인증·권한·비밀, DB/RLS/migration, 외부 write/deploy, 비가역 구조 변경은 구현 전에 설계와 롤백을 명시하고 해당 승인 경계를 확인한다.

API·스키마·RLS·인증 경계, 트랜잭션·재시도, 주요 의존성, 레이어·공개 계약의 선택은 작은 작업 면제 대상이 아니다. 장기 결정은 `docs/ai/decisions/`, 작업 설계는 `docs/ai/designs/`, 여러 단계 실행은 `docs/ai/plans/active/`에 기록하고, 완료 계획에는 실제 결과·검증·편차를 채워 `completed/`로 옮긴다. `docs/ai/`는 규칙을 복사하는 곳이 아니다. [원본 · .agents/WORKFLOW.md](../sources/workflow.md)

모델의 역량·가격과 추론 effort는 별도 축이다. 높은 역량의 모델도 하네스가 지원하면 `low` effort로 시작하고, 실제로 품질이 부족하거나 불확실성·실패 비용이 커질 때만 effort를 올린다. API 가격만으로 구독형 Codex·Claude Code의 모델 또는 effort를 고정하지 않고, 현재 선택 가능 모델·effort·한도를 확인한다. [원본 · .agents/WORKFLOW.md](../sources/workflow.md)

## 위임과 에스컬레이션

리드 에이전트가 범위·파일 소유권·통합·최종 검증을 맡고, 독립적으로 끝낼 수 있는 탐색·리뷰·테스트 조사 또는 겹치지 않는 모듈 구현만 위임한다. 공유 계약과 외부 쓰기 권한은 위임으로 넓어지지 않는다. 유효한 시도가 두 번 실패했거나, 선택지의 보안·호환성·비용·되돌림 차이가 크거나, API·데이터·권한 경계를 넘으면 사실·선택지·권장안·필요한 승인을 짧게 제시해 에스컬레이션한다. [원본 · .agents/WORKFLOW.md](../sources/workflow.md)
