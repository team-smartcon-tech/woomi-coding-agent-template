...
## 2026-09-14 · 수집 · 오늘의 판단 둘을 규칙으로 올렸다 (2.23 → 2.24-draft)

`/wiki-log-today` 를 돌려 오늘 한 것을 훑었더니 **커밋 4건의 기록은 다 있는데 정리본 반영이 0건**이었다. 그 명령이 "로그만 남기면 실패다 — 이 단계를 빼면 그냥 작업 일지가 된다" 고 말하는 자리다.

- **규칙은 위키가 아니라 `.agents/*` 로 보냈다.** 오늘 정한 둘은 "어떻게 동작하는가" 가 아니라 **앞으로 지켜야 할 기준**이라 규칙 문서가 소유해야 한다. 위키에 규칙을 적어 두면 아무도 안 지킨다.
  - `.agents/ui/DESIGN.md` 2장 — 본문 폭을 제한해 중앙 정렬하지 않는다(표·대시보드는 폭이 곧 정보량). 폼은 그 화면에서 감싼다. **실측 근거(1,574px 에서 `max-w-6xl` 이 한쪽 115px)를 규칙 옆에 적었다** — 근거 없는 규칙은 다음 사람이 되돌린다.
  - `.agents/ui/DESIGN.md` 4장 — 로고 위에 색을 덮지 않는다. 좁은 폭에서는 줄이지 말고 자산을 교체한다.
- **원본을 고쳤으니 정리본도 같은 작업에서 고쳤다** — [디자인 원칙](rules/design-principles.md). `wiki/CLAUDE.md` 가 "규칙 원본을 고쳤으면 같은 작업에서 대응 정리본도 고친다" 고 요구한다. 낡는 것은 도메인 페이지가 아니라 규칙 페이지다.
- **오늘 새로 만든 정리본은 없다.** 기존 페이지 갱신 1건뿐이고, 그게 맞다 — 오늘 알아낸 것이 이미 있는 주제(디자인 원칙)에 속했다. 새 페이지만 늘고 갱신이 0이면 흩어지고 있다는 신호인데, 오늘은 반대다.
- 미해결: 오늘 화면 작업 셋 다 **모바일 폭(`sm` 미만)에서 확인하지 않았다.** 데스크톱 1,574px 한 폭에서만 쟀다. 규칙에 모바일 조건을 적지 않은 것도 그래서다 — 재 보지 않은 것을 규칙으로 쓰지 않는다.
