...
- **본문 컨테이너에 폭 제한을 걸어 중앙 정렬하지 않는다.** 표·대시보드는 **폭이 곧 정보량**이다. `max-w-*` + `mx-auto` 를 걸면 넓은 화면에서 좌우에 빈 띠가 생기고 그만큼 손해다(실측: 1,574px 화면에서 `max-w-6xl` 이 한쪽 115px 을 먹었다).
- **폼처럼 넓으면 오히려 읽기 나쁜 화면은 그 화면에서 감싼다.** 전역 폭 제한 하나로 "표는 넓게, 폼은 좁게" 두 요구를 동시에 만족시킬 수 없다. 이름·이메일 입력이 1,200px 로 늘어나는 것은 `max-w-*` 를 그 라우트에 두어 막는다.
...
- **로고 이미지 위에 색을 덮지 않는다.** 브랜드 배지(`bg-primary`) 안에 넣거나 `filter`·`color` 로 물들이지 않는다. 로고 자산의 색과 UI 토큰(`--primary` 등)은 다른 값이고, 덮으면 브랜드 색이 깨진다. 나란히 놓여 미세하게 다른 색 두 개가 보이더라도 로고를 맞추지 않는다.
- **좁은 폭에서 가로형 로고를 줄이지 말고 자산을 교체한다.** 가로 3:1 워드마크를 접힘 사이드바(64px)에 넣으면 뭉개진다. 정사각 심볼 자산을 따로 두고 그 폭에서 바꾼다.
