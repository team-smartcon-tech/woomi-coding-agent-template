...
- 연결은 **명령 한 줄 + 브라우저 로그인**이다. 비밀키를 어디에도 붙여넣지 않는다.
- 읽기 전용으로만 연결한다. AI가 DB 내용을 바꾸지 못한다.
- 운영(production) DB에는 연결하지 않는다. 개발용 프로젝트만 연결한다.

**AI에게 이렇게 말하면 대신 해준다.**

```text
Supabase MCP 연결을 도와주세요.
.agents/TOOLING.md 6장 기준으로, 읽기 전용·개발 프로젝트 한정으로 설정해 주세요.
```

절차를 직접 보고 싶으면 [`.agents/TOOLING.md`](./TOOLING.md) 6장에 4단계로 적혀 있다. Supabase를 쓰지 않는 프로젝트는 이 절을 건너뛴다.
