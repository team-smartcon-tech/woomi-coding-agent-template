...
**다섯을 다 통과해도 화면에서 안 될 수 있다.** 기능(저장·편집·드래그·업로드·모드 전환)을 건드렸으면 PR 전에 브라우저에서 눌러 본다 — 절차는 [`docs/브라우저-검증-절차.md`](../../docs/브라우저-검증-절차.md). 파생 프로젝트에서 **테스트 490건이 전부 통과하는 상태로** 드래그가 잘못 동작한 사례가 있었다.
