...
- 규칙은 `AGENTS.md`와 `.agents/*`, 실행 중인 설계·계획은 `docs/ai/`, 탐색 가능한 팀 지식은 `wiki/`에 둔다. 같은 내용을 도구별 문서나 위키에 복제하지 않는다.
...
작업을 시작할 때 아래 중 가장 높은 등급을 판단해, 필요한 만큼만 계획과 검토를 늘린다.

| 등급 | 예시 | 필요한 절차 |
|---|---|---|
| 낮음 | 문구·스타일·명확한 단일 파일 버그 | 대상 코드 확인 후 구현·검증 |
| 중간 | 여러 모듈, 새 기능, API 소비자 변경, 원인 불명확한 실패 | 영향·계약·검증을 짧게 계획하고 관련 문서 확인 |
| 높음 | 인증·권한·secret, DB/RLS/migration, 외부 write/deploy, 비가역 구조 변경 | 구현 전에 설계와 롤백을 명시하고 해당 영역 규칙·승인 경계 확인 |

다음은 작은 작업 면제 대상이 아니다: API·스키마·RLS·인증 경계, 트랜잭션·재시도, 주요 의존성, 레이어·공개 계약을 바꾸는 결정. 선택지와 근거, 불변조건, 위험·롤백, 검증을 구현 전에 기록한다. 장기적으로 남길 결정은 `docs/ai/decisions/`, 작업별 설계는 `docs/ai/designs/`, 여러 단계 실행은 `docs/ai/plans/active/`에 둔다. 완료한 계획에는 결과·검증·계획과의 차이를 채워 `docs/ai/plans/completed/`로 옮긴다.

`wiki/`가 있으면 관련 이전 결정·해결책을 먼저 찾고, 재사용할 판단이나 해결 과정이 생길 때만 출처 링크와 함께 기록한다. 위키는 설계·계획의 사본이 아니라 발견과 재사용 지식의 색인이다.

### 2.2 모델과 추론 effort

모델의 역량·가격과 추론 effort는 별도 축이다. 높은 역량의 모델을 골랐더라도, 도구가 effort를 제공하면 기본은 `low`로 시작한다. `low`에서 필요한 품질을 얻지 못했거나, 고위험 결정·비자명한 원인 분석·여러 설계안의 비교가 실제로 필요할 때만 `medium` 이상으로 올린다.

따라서 “최상위 모델 = 높은 추론”으로 고정하지 않는다. 긴 컨텍스트·도구 능력·정확성이 필요한 작업에는 최상위 모델의 `low` effort를 쓸 수 있고, 실패 비용과 불확실성이 늘어날 때만 effort를 올린다. 실제 선택 가능한 모델·effort·사용량 한도는 각 하네스의 현재 모델 선택 화면 또는 상태 명령으로 확인한다.
...
리드 에이전트는 범위, 파일 소유권, 통합과 최종 검증을 책임진다. 위임은 독립적으로 끝낼 수 있는 탐색, 리뷰, 테스트 조사 또는 서로 겹치지 않는 모듈 구현에만 쓴다. 공유 파일을 여러 에이전트가 동시에 수정하게 하지 않고, 외부 쓰기·배포·운영 데이터 변경 권한도 위임으로 넓어지지 않는다.

위임 요청에는 목표, 읽을 문서·파일, 수정 가능 범위, 금지 범위, 완료 조건과 검증을 적는다. 결과는 변경 파일, 근거, 검증, 미확인 위험을 포함해 리드에게 돌려준다.

다음 때는 더 높은 검토 또는 사용자 판단으로 에스컬레이션한다.

- 유효한 조사·수정 시도가 두 번 실패했거나 원인이 불명확할 때
- 선택지 간 보안·호환성·비용·되돌림의 차이가 실질적일 때
- API/데이터/권한 경계를 넘거나 비가역 작업을 앞둘 때

에스컬레이션은 추측으로 결론 내리지 말고, 확인한 사실·선택지·권장안·필요한 승인을 짧게 제시한다.
